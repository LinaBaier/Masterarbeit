# Classifier to detect morphed images from their barcode features
# #############################################################################################
# We use barcode intervals of single images determined in javaplex.
# For each image we read off features from those barcode intervals. --> feature vector
# For each image we know whether it is morphed or original. --> label
# We apply random forest classifier to detect morphed images.
...
##################################################################################################
# Determine useful features from barcodes --> barcodes as results of javaplex
# Useful features:  number of bars in dimension 0, 1, 2
#                   length of longest bar in dimension 1, 2
#                   birth of longest bar in dimension 1, 2
#                   length of second longest bar in dimension 0
...
# Apply random forest to labelled data using the feature vectors from above
...
# Modify test data set (random deletion of data from morphed images)
...
# Use crossfold method for selecting training and test data
...
def morph_detector(path_morphs, path_originals):
    ...
    results = results_morphs.append(results_originals).reset_index(drop=True).drop_duplicates("Image") # to make sure that the results are not caused by using the same picture several times
